...
from em_ebay_repricer.exchange_rate import ExchangeRate
...
class PriceCalculator:
    ...
    def calc_offer(self, src_offer):
        if isinstance(src_offer, bool) and not src_offer:
            return False
        if src_offer is None or src_offer.get("price", 0) == 0:
            return {
                "price": 0,
                "quantity": 0,
        ...
        price_in_usd_amount = (
            src_price_in_usd + self.rules["transfer_cost"] + self.min_profit_amount
        )
        price_in_usd_margin = (
            self.rules["ad_cost"]
            + self.rules["transfer_cost"]
            + src_price_in_usd * (1 + self.rules["tax_rate"])
        ) / (1 - self.rules["margin"])
        ...
        price_by_product_cost = (
            cost / self.rules["product_cost_rate"]
            + self.rules["ad_cost"]
            + self.rules["transfer_cost"]
        )
        price_by_product_cost = max(
            price_in_usd_amount * self.exchange_rate, price_by_product_cost
        )
        price = (
            price_by_product_cost
            if price_by_margin > price_by_product_cost
            else price_by_margin
        )
        ...
        if src_quantity:
            ...
            availability_type = src_offer.get("shipping_time", {}).get(
                "availability_type", None
            )
            fba = src_offer.get("fba", False)
            if (
                availability_type
                and availability_type.lower().find("now") == -1
                and not fba
            ):
                quantity = 0
            else:
